...
def plot(year):
    df = pd.read_csv('data/go/clean.csv')
    df['money'] = df.apply(lambda r: r['金额'] if r['收支']
                           == '收入' else -r['金额'], axis=1)
    stats = df.groupby(['年份', '月份', '分类', '子分类']).sum('money').reset_index()

    plot_month_line(stats[stats['年份'] == year], f'{
                    year}.png', f'{year}年各类支出情况')
...
def plot_year_line(df: pd.DataFrame, save_path, title=None):
    param = {
        'df': df,
        'save_path': save_path,
        'width': 1,
...
def sub_plot(df, save_path, width, cat, x, y, hue=None, figsize=(5, 4), title=None):
    ...
    fig, axes = plt.subplots(height, width, figsize=(
        figsize[0]*width, figsize[1]*height))
